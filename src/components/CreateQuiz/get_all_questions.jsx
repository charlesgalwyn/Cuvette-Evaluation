...
const GetAllQuestions = ({ setUrl, changePopup, quizeData, type }) => {
  ...
  const submitCreateQuize = async () => {
    try {
      const { data } = await axios.post('http://localhost:4000/api/quize/create', quizeData, {
        headers: {
  ...
  const submitEditQuize = async () => {
    try {
      const { data } = await axios.put(`http://localhost:4000/api/quize/${quizeData._id}`, quizeData, {
        headers: {
          Authorization: localStorage.getItem('authToken')
        }
      })
